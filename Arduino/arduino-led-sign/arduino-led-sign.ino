...
#define STARTING_BRIGHTNESS  85
#define FADE_IN_SPEED        15
#define FADE_OUT_SPEED       7
#define DENSITY              255
...
// Used to maintain on/off state. false == off, true == on
...
void setup() {
    ...
    server.on("/status", signStatus);
    server.on("/toggle", toggleSign);
...
void signStatus() {
    if (currentState == 0) {
        server.send(200, "text/plain", "sign-off");
    } else {
        server.send(200, "text/plain", "sign-on");
        }
}

void toggleSign() {
    Serial.println("Sign is being toggled...");

    // Toggle sign state
    changeState();

    // If the sign is being toggled from on -> off 
    if (!currentState) {
    stopLEDs();
    }
